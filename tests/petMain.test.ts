...
describe('pet-main interactions', () => {
  ...
  it('handles a focus session completed event with a completion message bubble', () => {
    ...
    expect(source).toContain(
      'const timer = window.setTimeout(() => setFocusCompletionMessage(null), FOCUS_COMPLETION_MESSAGE_DURATION_MS)'
    )
  })
  ...
  it('reports the required pet window width so long reminder bubbles can expand without scrollbars', () => {
    const source = readFileSync(join(process.cwd(), 'src/renderer/src/pet-main.tsx'), 'utf8')
    const helperSource = readFileSync(join(process.cwd(), 'src/renderer/src/petWindowContentWidth.ts'), 'utf8')
  ...
  it('keeps the focus countdown bubble visible and frozen while a break pauses focus', () => {
    ...
    expect(source).toContain('formatDuration(Math.ceil(payload.focusSession.pausedRemainingMs / 1000))')
