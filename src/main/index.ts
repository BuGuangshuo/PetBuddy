...
import { getAccessibilityStatus, openAccessibilitySettings, promptForAccessibilityIfNeeded } from './services/permissions'
...
const assetRoot = app.isPackaged ? join(process.resourcesPath, 'pet_assets') : join(process.cwd(), 'pet_assets')
...
const main = async (): Promise<void> => {
  ...
  const readFocusSessionState = () => {
    ...
    if (activeFocusSession && !isFocusSessionActive(activeFocusSession, now)) {
      const elapsedFocusSeconds = getElapsedFocusSessionSeconds(activeFocusSession, now)
  ...
  const activateFocusDoneState = (now: number): void => {
    ...
    focusDoneTimer = setTimeout(() => clearFocusDoneState(), FOCUS_DONE_DURATION_MS)
  }
  ...
  const startFocusSession = (): SettingsPayload => {
    ...
    focusSessionTimer = setTimeout(() => completeFocusSession({ showBubble: true }), durationMs)
    emitPetEvent(windows.petWindow, {
  ...
  const emitAppearanceChanged = (appearanceId: AppSettings['selectedPetAppearance']): void => {
    const appearance = appearances.find((item) => item.id === appearanceId) ?? appearances[0]
    if (appearance) {
      emitPetEvent(windows.petWindow, { type: 'appearance-changed', appearance })
    }
  }
  ...
  const refreshSelectedAppearanceIfNeeded = (appearanceId: AppSettings['selectedPetAppearance']): void => {
  ...
  const reminderService = new ReminderService({
    ...
    onReminderFinished: (event) => {
      emitPetEvent(windows.petWindow, { type: 'reminder-finished', eventId: event.id })
    }
  })
  ...
  const updateSettings = async (patch: Partial<AppSettings>): Promise<SettingsPayload> => {
    let nextPatch = patch
    ...
    if (patch.petPosition) {
      ...
      emitPetEvent(windows.petWindow, { type: 'position-changed', position: nextSettings.petPosition })
    }
  ...
  registerIpc({
    ...
    toggleFocusMode: async (enabled: boolean) => updateSettings({ focusModeEnabled: enabled }),
