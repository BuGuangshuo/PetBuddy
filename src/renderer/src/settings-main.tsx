...
import './styles.css'
import { resolveAppearanceCustomizePreviewAsset, resolveAppearancePreviewAsset } from './sceneAssetResolver'
      ...
      <div className="chips">
        {values.map((value) => (
          <span className="chip" key={value}>
            {value}
            <button onClick={() => onChange(values.filter((item) => item !== value))}>×</button>
          </span>
        ))}
      </div>
      <input
...
const AppearanceCard = ({
  ...
  <button className={`appearance-card ${active ? 'active' : ''}`} onClick={onSelect}>
    <div className="appearance-preview">
      <img src={resolveAppearancePreviewAsset(appearance)} alt={appearance.displayName} />
    </div>
    <div className="appearance-name">{appearance.displayName}</div>
    <div className="appearance-meta">{appearance.id === 'golden-puppy' ? 'beta' : '已启用素材'}</div>
  </button>
)
...
const SettingsApp = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.petBuddy.pet.onEvent((event) => {
      ...
      if (event.type === 'focus-session-updated') {
        setPayload((current) => (current ? { ...current, focusSession: event.focusSession } : current))
      }
    })
  ...
  const selectedAppearance = useMemo(
    () => payload?.appearances.find((item) => item.id === payload.settings.selectedPetAppearance) ?? null,
    [payload]
  )
  ...
  const activeFocusSeconds =
    payload.focusSession.status === 'active'
      ...
      : payload.focusSession.status === 'paused'
        ? 0
      : 0
  const displayedFocusMinutes = Math.floor((stats.focusDurationSeconds + activeFocusSeconds) / 60)
      ...
      <div className="settings-shell">
        ...
        <div className="hero">
          <div className="hero-mark">
            {selectedAppearance ? <img src={resolveAppearancePreviewAsset(selectedAppearance)} alt="" /> : null}
          </div>
          <div>
            <div className="hero-eyebrow">PAWPAL</div>
            <h1 className="hero-title">今日</h1>
          </div>
        </div>

        <div className="stats-grid">
          <StatCard label="休息" value={stats.shown.break} unit="次" />
          <StatCard label="喝水" value={stats.acknowledged.water} unit="次" />
          <StatCard label="专注" value={displayedFocusMinutes} unit="分钟" />
          <StatCard label="分心" value={stats.shown.focusNudge} unit="次" />
        </div>

        {(showOnboarding || payload.permissionState !== 'granted') ? (
          <div className="notice">
            <div>
              <strong>欢迎使用 PawPal</strong>
              <p>
                PawPal 会待在菜单栏和屏幕上，定时提醒你休息、喝水和保持节奏。
            ...
            <div className="inline-row">
              {payload.permissionState !== 'granted' ? (
                <button className="ghost-button" onClick={() => void window.petBuddy.permissions.openAccessibilitySettings()}>
                  打开权限设置
                </button>
              ) : null}
              {showOnboarding ? (
                <button className="ghost-button" onClick={() => void updateSettings({ onboardingCompleted: true })}>
                  知道了
                </button>
              ) : null}
            </div>
        ...
        <section className="section">
          ...
          <div className="section-panel">
            ...
            <div className="field-row">
              <div>
                <div className="field-label">宠物形象</div>
              </div>
              <div className="field-control">
                  ...
                  {payload.appearances.map((appearance) => (
                    <AppearanceCard
                      ...
                      onSelect={() => {
                        ...
                        void updateSettings({ selectedPetAppearance: appearance.id })
                      }}
                    />
                  ))}
                  <button
                  ...
                  >
                    <div className="appearance-preview">
                      {customizePreviewAsset ? (
                        <img src={customizePreviewAsset} alt={`${selectedAppearance?.displayName ?? '当前宠物'} 自定义预览`} />
                      ) : (
                        <div style={{ fontSize: 28 }}>＋</div>
                      )}
                    </div>
                    <div className="appearance-name">自定义</div>
                    <div className="appearance-meta">编辑当前宠物的场景素材</div>
                  </button>
                </div>
              </div>
            </div>
            ...
            {selectedAppearance && isAppearanceCustomizing ? (
              <div className="field-row">
                <div>
                  ...
                  <div className="field-hint">正在编辑 {selectedAppearance.displayName} 的场景 GIF。</div>
                </div>
                <div className="field-control">
                  <div className="scene-asset-list">
                    {selectedAppearance.sceneAssets.filter((sceneAsset) => sceneAsset.scene !== 'sit').map((sceneAsset) => {
                      const referenceSceneAsset = referenceAppearance?.sceneAssets.find((item) => item.scene === sceneAsset.scene)

                      return (
                        <div className="scene-asset-row" key={sceneAsset.scene}>
                          <div className="scene-asset-copy">
                            <div className="scene-asset-name">{sceneAsset.label}</div>
                            <div className="scene-asset-desc">{sceneAsset.description}</div>
                          </div>
                          <div className="scene-asset-previews">
                            <div className="scene-preview-card">
                              <div className="scene-preview-label">内置</div>
                              <div className="scene-preview-frame">
                                {referenceSceneAsset?.defaultAssets[0] ? (
                                  <img src={referenceSceneAsset.defaultAssets[0]} alt={`${sceneAsset.label} 内置`} />
                                ) : (
                                  <span>暂无</span>
                                )}
                              </div>
                            ...
                            <div className="scene-preview-card">
                              <div className="scene-preview-label">自定义</div>
                              <div className="scene-preview-frame">
                                {sceneAsset.customAsset ? <img src={sceneAsset.customAsset} alt={`${sceneAsset.label} 自定义`} /> : <span>未上传</span>}
                              </div>
                            </div>
                          </div>
                          <div className="scene-asset-actions">
                            <button
                              ...
                              onClick={() =>
                                void runPayloadAction(
                                  window.petBuddy.appearanceScenes.uploadCustomGif(selectedAppearance.id, sceneAsset.scene)
                                )
                              }
                            >
                            ...
                            {sceneAsset.customAsset ? (
                              <button
                                ...
                                onClick={() =>
                                  void runPayloadAction(
                                    window.petBuddy.appearanceScenes.clearCustomGif(selectedAppearance.id, sceneAsset.scene)
                                  )
                                }
                              >
                                清除
          ...
          <div className="section-panel">
            <div className="field-row">
              ...
              <div className="field-control">
                <Toggle
                  ...
                  onChange={(checked) => void updateSettings({ breakIntervalMinutes: nextReminderInterval(checked, 50) })}
                />
              </div>
            </div>
            {isBreakReminderEnabled ? (
              <div className="field-row">
                ...
                <div className="field-control">
                  <Stepper value={payload.settings.breakIntervalMinutes} unit="分钟" min={1} max={120} step={1} onChange={(next) => void updateSettings({ breakIntervalMinutes: next })} />
                </div>
              </div>
            ) : null}
            <div className="field-row">
              ...
              <div className="field-control">
                <Toggle
                  ...
                  onChange={(checked) => void updateSettings({ waterIntervalMinutes: nextReminderInterval(checked, 30) })}
                />
              </div>
            </div>
            {isWaterReminderEnabled ? (
              <div className="field-row">
                ...
                <div className="field-control">
                  <Stepper value={payload.settings.waterIntervalMinutes} unit="分钟" min={1} max={180} step={1} onChange={(next) => void updateSettings({ waterIntervalMinutes: next })} />
                </div>
              </div>
            ) : null}
          </div>
            ...
            <div className="field-row">
              ...
              <div className="field-control">
                <Stepper
                  ...
                  max={180}
                  step={5}
                  onChange={(next) => void updateSettings({ focusSessionMinutes: next })}
                />
              </div>
            </div>
            <div className="field-row">
              <div>
                ...
                <div className="field-hint">检测当前前台 App；命中预设分心应用时，会在宽限时间后提醒你回到工作。</div>
              </div>
              <div className="field-control">
                <Toggle
                  checked={isFocusModeEnabled}
                  onChange={(checked) => void runPayloadAction(window.petBuddy.app.toggleFocusMode(checked))}
                />
              </div>
            </div>
            {isFocusModeEnabled ? (
              <>
                <div className="field-row">
                  ...
                  <div className="field-control">
                    <Stepper value={payload.settings.focusGraceSeconds} unit="秒" min={5} max={60} step={5} onChange={(next) => void updateSettings({ focusGraceSeconds: next })} />
                  </div>
                </div>
                <div className="field-row">
                  <div>
                    <div className="field-label">分心应用</div>
                    <div className="field-hint">输入 App 名或 bundle id，按 Enter 添加。</div>
                  </div>
                  <div className="field-control">
                    <ChipEditor
                      values={payload.settings.distractingApps}
                      onChange={(next) => void updateSettings({ distractingApps: next })}
          ...
          <div className="section-panel">
            <div className="field-row">
              <div>
                ...
                <div className="field-hint">正式打包版会在登录后启动；开发环境仅保存偏好。</div>
              </div>
              <div className="field-control">
                <Toggle checked={payload.settings.launchAtLogin} onChange={(checked) => void updateSettings({ launchAtLogin: checked })} />
              </div>
            </div>
            <div className="field-row">
              <div>
                <div className="field-label">启动时检查更新</div>
                <div className="field-hint">关闭后只会在你手动检查时访问 GitHub Releases。</div>
              </div>
              <div className="field-control">
                <Toggle checked={payload.settings.checkUpdatesOnStartup} onChange={(checked) => void updateSettings({ checkUpdatesOnStartup: checked })} />
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="section-panel">
            <div className="about-grid">
              ...
              <div className="about-row">
                ...
                <button className="secondary-button" onClick={() => void window.petBuddy.updates.checkNow().then(() => void load())}>
                  检查更新
                </button>
              </div>
              <div className="about-row">
                <div>
                  ...
                  <div className="about-text">在浏览器中打开 GitHub Releases。</div>
                </div>
                <button className="secondary-button" onClick={() => void window.petBuddy.updates.openReleasesPage()}>
                  打开 Releases
                </button>
              </div>
              <div className="about-row">
                <div>
                  ...
                  <div className="about-text">{payload.isMacArm64 ? 'Apple Silicon macOS' : '当前平台不在正式支持范围内'}</div>
                </div>
